...
<hr>
### AdvAgg 
**Issue**: AdvAgg can cause conflicts with the platform Varnish caching leading to vearious errors, typically seen as @Adv CSS/JS Agg – HTTP Request	Error	HTTP requests to advagg for css files are not getting through.@ and@Adv CSS/JS Agg – HTTP Request	Error	HTTP requests to advagg for js files are not getting through.@.
